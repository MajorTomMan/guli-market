...
CREATE DATABASE gulimall_ums default CHARACTER set utf8mb4;
...
CREATE TABLE `ums_member` (
    ...
    `expires_in` datetime DEFAULT NULL COMMENT '访问令牌的时间',
    `from_qq` boolean DEFAULT false COMMENT '来自QQ',
    `from_github` boolean DEFAULT false COMMENT '来自github',
    `from_weixin` boolean DEFAULT false COMMENT '来自微信',
    `from_weibo` boolean DEFAULT false COMMENT '来自微博',
    PRIMARY KEY (`id`)
...
CREATE TABLE `ums_member_github` (
    ...
    gravatar_id VARCHAR(255) COMMENT 'Gravatar ID',
    url VARCHAR(255) COMMENT '用户的 API URL',
    html_url VARCHAR(255) COMMENT '用户个人资料的 HTML URL',
    followers_url VARCHAR(255) COMMENT '用户的关注者 API URL',
    following_url VARCHAR(255) COMMENT '用户关注的用户 API URL',
    gists_url VARCHAR(255) COMMENT '用户的 Gists API URL',
    starred_url VARCHAR(255) COMMENT '用户收藏的库的 API URL',
    subscriptions_url VARCHAR(255) COMMENT '用户的订阅 API URL',
    organizations_url VARCHAR(255) COMMENT '用户所属组织的 API URL',
    repos_url VARCHAR(255) COMMENT '用户的库 API URL',
    events_url VARCHAR(255) COMMENT '用户的事件 API URL',
    received_events_url VARCHAR(255) COMMENT '用户接收到的事件 API URL',
    type VARCHAR(50) COMMENT '用户类型（例如：用户、组织）',
    site_admin BOOLEAN COMMENT '用户是否为网站管理员',
    name VARCHAR(255) COMMENT '用户的全名',
    company VARCHAR(255) COMMENT '用户所在公司',
    blog VARCHAR(255) COMMENT '用户的博客 URL',
    location VARCHAR(255) COMMENT '用户的位置信息',
    email VARCHAR(255) COMMENT '用户的电子邮件地址',
    hireable BOOLEAN COMMENT '用户是否可被雇佣',
    bio TEXT COMMENT '用户的个人简介',
    twitter_username VARCHAR(255) COMMENT '用户的 Twitter 用户名',
    notification_email VARCHAR(255) COMMENT '用户的通知电子邮件',
    public_repos INT COMMENT '公共库数量',
    public_gists INT COMMENT '公共 Gists 数量',
    followers INT COMMENT '关注者数量',
    following INT COMMENT '用户关注的数量',
    created_at TIMESTAMP COMMENT '账户创建时间戳',
    updated_at TIMESTAMP COMMENT '最后更新时间戳'
